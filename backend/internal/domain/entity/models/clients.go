...
var ClientWhere = struct {
	...
	CompanyID: whereHelperint64{field: "`clients`.`company_id`"},
...
func (r *clientR) GetCompany() *Company {
	if r == nil {
		return nil
	}
	return r.Company
...
func (r *clientR) GetBankAccounts() BankAccountSlice {
	if r == nil {
		return nil
	}
...
var (
	clientAllColumns            = []string{"id", "company_id", "name", "phone", "address"}
	clientColumnsWithoutDefault = []string{"company_id", "name", "phone", "address"}
...
func (o *Client) Company(mods ...qm.QueryMod) companyQuery {
...
func (o *Client) BankAccounts(mods ...qm.QueryMod) bankAccountQuery {
	...
	queryMods = append(queryMods,
		qm.Where("`bank_accounts`.`client_id`=?", o.ID),
	)
...
func (clientL) LoadCompany(ctx context.Context, e boil.ContextExecutor, singular bool, maybeClient interface{}, mods queries.Applicator) error {
	var slice []*Client
	var object *Client
...
func (clientL) LoadBankAccounts(ctx context.Context, e boil.ContextExecutor, singular bool, maybeClient interface{}, mods queries.Applicator) error {
	var slice []*Client
	var object *Client
	...
	query := NewQuery(
		qm.From(`bank_accounts`),
		qm.WhereIn(`bank_accounts.client_id in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	results, err := query.QueryContext(ctx, e)
	if err != nil {
		return errors.Wrap(err, "failed to eager load bank_accounts")
	}

	var resultSlice []*BankAccount
	if err = queries.Bind(results, &resultSlice); err != nil {
		return errors.Wrap(err, "failed to bind eager loaded slice bank_accounts")
	}

	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results in eager load on bank_accounts")
	}
	if err = results.Err(); err != nil {
		return errors.Wrap(err, "error occurred during iteration of eager loaded relations for bank_accounts")
	}
...
func (clientL) LoadInvoices(ctx context.Context, e boil.ContextExecutor, singular bool, maybeClient interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		qm.From(`invoices`),
		qm.WhereIn(`invoices.client_id in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
// SetCompany of the client to the related item.
// Sets o.R.Company to related.
// Adds o to related.R.Clients.
func (o *Client) SetCompany(ctx context.Context, exec boil.ContextExecutor, insert bool, related *Company) error {
	var err error
	if insert {
...
func (o *Client) AddBankAccounts(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*BankAccount) error {
	var err error
